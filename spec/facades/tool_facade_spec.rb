...
RSpec.describe ToolFacade do
  let(:tools_response) { File.read('spec/fixtures/tools_index.json') }
  let(:tool_facade) { ToolFacade.new }
  ...
  it "exists and returns attributes" do 
    ...
    expect(tools[0].borrow_id).to eq("33")
    ...
    expect(tools[0].location).to eq("123 Sunnyside Dr, Lebanon, IN, 46052")
